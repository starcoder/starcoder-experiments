...
def random_tree(min_count, max_count, unary_prob, prefix, mu, sigma):
    consts = generate_constants(min_count, max_count, mu, sigma)
...
def generate_constants(minimum_constants, maximum_constants, mu, sigma):
    num_consts = random.randint(minimum_constants, maximum_constants)
    consts = [abs(random.gauss(mu, sigma)) for _ in range(num_consts)]
    nodes = []
...
def populate_tree(node, prefix=""):
    if isinstance(node, ast.Expression):
        ...
        node.relations = {"left_arg" : node.left.id, "right_arg" : node.right.id}
        ...
        node.relations = {"unary_arg" : node.operand.id}
    elif isinstance(node, ast.Num):
        node.value = node.n
        node.id = prefix
        node.name = "const"
