...
if __name__ == "__main__":
    ...
    parser.add_argument("-c", "--components", dest="components", type=int, default=10000, help="Number of graph components to generate")
    ...
    parser.add_argument("--mu", dest="mu", type=float, default=10.0, help="")
    parser.add_argument("--sigma", dest="sigma", type=float, default=2.0, help="")    
    args, rest = parser.parse_known_args()

    with gzip.open(args.output, "wt") as ofd:
        for component_id in range(1, args.components + 1):
            for node in random_tree(args.minimum_constants, args.maximum_constants, args.unary_probability, prefix="{}".format(component_id), mu=args.mu, sigma=args.sigma):
